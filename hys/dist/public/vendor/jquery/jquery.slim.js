...
(function (global, factory) {
    ...
    (function (window) {
        ...
        rwhitespace = new RegExp(whitespace + '+', 'g'), rtrim = new RegExp('^' +
            ...
            '+|((?:^|[^\\\\])(?:\\\\.)*)' +
            whitespace +
            '+$', 'g'), rcomma = new RegExp('^' + whitespace + '*,' + whitespace + '*'), rcombinators = new RegExp('^' +
            whitespace +
            '*([>+~]|' +
            whitespace +
            ')' +
...
---------------------------------------------------------------------- */
                ...
                CHILD: function (type, what, argument, first, last) {
                    ...
                    return first === 1 && last === 0
                        ...
                        : function (elem, context, xml) {
                            var cache, uniqueCache, outerCache, node, nodeIndex, start, dir = simple !== forward
                                ? 'nextSibling'
                                : 'previousSibling', parent = elem.parentNode, name = ofType && elem.nodeName.toLowerCase(), useCache = !xml && !ofType, diff = false;
                            if (parent) {
    ...
    function boxModelAdjustment(elem, dimension, box, isBorderBox, styles, computedVal) {
        ...
        if (!isBorderBox && computedVal >= 0) {
            ...
            delta +=
                Math.max(0, Math.ceil(elem['offset' +
            ...
            set: function (elem, value, extra) {
                ...
                boxSizingNeeded = scrollboxSizeBuggy || extra, isBorderBox = boxSizingNeeded &&
                    jQuery.css(elem, 'boxSizing', false, styles) ===
                        'border-box', subtract = extra
                    ? boxModelAdjustment(elem, dimension, extra, isBorderBox, styles)
                    : 0;
                // Account for unreliable border-box dimensions by comparing offset* to computed and
                // faking a content-box to get border and padding (gh-3699)
                if (isBorderBox && scrollboxSizeBuggy) {
                    subtract -= Math.ceil(elem['offset' +
                        dimension[0].toUpperCase() +
                        dimension.slice(1)] -
    ...
    jQuery.extend(jQuery.event, {
        trigger: function (event, data, elem, onlyHandlers) {
            var i, cur, tmp, bubbleType, ontype, handle, special, lastElement, eventPath = [elem || document], type = hasOwn.call(event, 'type') ? event.type : event, namespaces = hasOwn.call(event, 'namespace')
